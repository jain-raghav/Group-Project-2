...
Telco has hired us to examine factors impacting customer decision making with the goal of reducing churn, and in this report we will take a deeper dive into the relative influence of said factors. Our ultimate goal is to be able to accurately predict which customers will churn so we can target them with a promotion or incentive to retain their business.
...
We see there are 11 data points which are not complete - indicated by the 11 NA's in the Total Charges five number summary. Accordingly, we decided to remove the 11 rows with missing data. This should not affect results as there are more than 7500 rows of data and only 11 out of those are being omitted.
...
Moreover, certain variables, such as Customer ID and Class, are not predictive. Instead, these are simply used to identify the various customers of the company. Therefore, these variables were removed as they should not be significant in our predictive models.
...
## Models
In this following section, we are going to generate models with the goal of predicting customer churn and better understanding the significant predictors.  

### Logistic Regression Model
The first model we created was a logistic regression model. This model uses a logistic function to model a binary response variable based on explanatory variables. We wanted to create this initial model to help us understand if there were any insignificant variables that we should be removing from our models and also to get an idea of which predictors are stronger than others. We also optimized this model using a backwards step wise function.
...
The models we created could be useful to the Telco company by predicting which current customers are most likely to churn, and thereby giving the firm an opportunity to target those customers with price promotions and/or incentives to stay. Looking at our models we saw that they all performed relatively equally, with the tree model performing the best with a 77.4% accuracy.

The next step of this analysis would be to get more information from the Telco provider regarding the cost of such a promotion and the total lifetime value of a customer, so that calculations can be done to determine how much revenue could be preserved. In this step, the sensitivity (true positive) and specificity (true negative) values of each model would be important to understanding costs. For example, if the cost of retaining a customer is just barely lower than retaining them, it would be important that no unneeded promotions are given out. In this case, a model with high sensitivity and specificity may be preferable to one with a high overall accuracy. In our case, we can assume that the cost of a promotion which could be anywhere from $10-50 dollars for example which should be significantly smaller than the cost of a customer churning which could several hundred dollars. This means that the cost of a false positive(customer predicted to leave but doesn't) is way lower than the cost of a false negative(customer churned but not predicted to). It is also important to note that not all customers who are offered a promotion are guaranteed to stay, so it could be important for Telco to understand how effective the promotions are in order to price them effectively. Ideally, the promotions are given to customers who are going to churn, preventing them from stopping service and therefore increasing revenues for Telco.

Telco can also benefit from understanding which factors are significant predictors for churn like Internet Service, Monthly Charges, Contract Length, etc. This can give them information on such as plans customers like and which plans customers are leaving from or even which products/services customers like and which ones they don't like. Using this information, they can improve upon their business by reworking plans that don't perform well and making changes to poorly performing products.


Once the model(s) are put into use, we expect Telco's churn numbers to go down, indicated by higher customer retention metrics (controling for things such as customers moving away, death, etc.). Since many models performed similarly, we recommend that the Telco company deploy multiple models on random subsets of customers, so that the models can be forward-tested for a bit to determine which truly performs the best in practice. Additionally, different models provide us with slightly different information, so it could be important to pull from a combination of them.
